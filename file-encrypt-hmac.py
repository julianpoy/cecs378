...
#encrypt with HMAC
def MyencryptMAC(message, key, hmac):
  #begin encrypt then mac
  #ENCRYPT THEN HMAC!
  iv = os.urandom(IVLength)
  h = hmac.HMAC(key, hashes.SHA256(), backend = default_backend())
  #Creates an encryptor in the specified mode with the given iv and key
...
def MyfileEncryptMAC(filepath):
  #generates key
  key = os.urandom(filepath)
  #generates the hmac
  h = hmac.HMAC(key, hashes.SHA256(), backend=default_backend())
  #Retrieves the file extension
  file_name, file_extension = os.pathsplittext(filepath)
  #Opens the file specified at the file path in mode read binary
  with open(filepath, "rb") as binary_file:
    # Read the whole file at once
    data = binary_file.read()
    #Encrypts the file
    iv, ciphertext, tag = MyencryptMAC(data,key,h)
...
if'--encryptmac' in sys.argv:
  #The message to be encrypted with the hmac
  filepath = sys.argv[sys.argv.index('--encryptmac') + 1]
  #calls the encrypt_file on the filepath
  key , message = MyfileEncryptMAC(filepath)
